...
foreach my $arg (@ARGV)
{
	...
	elsif ($arg eq "selectTouching")	{	our $selectTouching = 1;			}
	elsif ($arg eq "selectOnObject")	{	our $selectOnObject = 1;			}
	elsif ($arg eq "selectAll")			{	our $selectAll = 1;					}
	else								{	$facingRatio = cos($arg*($pi/180));	}
}
