...
SMO_RIZOMUV_LIVELINK_HOTKEYS = [
    {
        "contexts": [
            [".global", "(stateless)", ".anywhere", "(contextless)", "attr.formPopover {SMO_LL_RIZOMUV_PM_SH:sheet}"]
        ],
        "key": "ctrl-alt-shift-r",
        "command": "attr.formPopover {SMO_LL_RIZOMUV_PM_SH:sheet}",
        "name": "RIZOM UV LiveLink: Pie Menu",
        "info": "Ctrl Alt Shift R"
    }

]

# max = len(SMO_RIZOMUV_LIVELINK_HOTKEYS)
# lx.out(max)


class SMO_RIZOMUV_LIVELINK_KeymapCmdClass(SmoCommanderClass):
    def commander_arguments(self):
        args = []
        for n, hotkey in enumerate(SMO_RIZOMUV_LIVELINK_HOTKEYS):
            args.append({
                'name':str(n),
    ...
    def commander_execute(self, msg, flags):
        ...
        # modo.dialogs.alert("Mapped SMO MARMOSET LiveLink Hotkeys", "Mapped %s SMO MARMOSET LiveLink hotkeys. See Help > Smonster Hotkey Reference" % n)

lx.bless(SMO_RIZOMUV_LIVELINK_KeymapCmdClass, "smo.LL.RIZOMUV.MapDefaultHotkeys")


class RemoveSMO_RIZOMUV_LIVELINK_KeymapCmdClass(SmoCommanderClass):

    def commander_execute(self, msg, flags):
        for hotkey in SMO_RIZOMUV_LIVELINK_HOTKEYS:
            key = hotkey["key"]
        ...
        modo.dialogs.alert("Reverted SMO RIZOM UV LiveLink Hotkeys", "Reverted %s SMO RIZOM UV LiveLink hotkeys to defaults." % len(SMO_RIZOMUV_LIVELINK_HOTKEYS))

lx.bless(RemoveSMO_RIZOMUV_LIVELINK_KeymapCmdClass, "smo.LL.RIZOMUV.UnmapDefaultHotkeys")


class ClearSMO_RIZOMUV_LIVELINK_KeymapCmdClass(SmoCommanderClass):
    ...
    def commander_execute(self, msg, flags):
        ...
        modo.dialogs.alert("Cleared SMO RIZOM UV LiveLink Hotkeys", "Cleared %s SMO RIZOM UV LiveLink hotkeys attribution." % len(SMO_RIZOMUV_LIVELINK_HOTKEYS))

lx.bless(ClearSMO_RIZOMUV_LIVELINK_KeymapCmdClass, "smo.LL.RIZOMUV.ClearHotkeys")