...
Purpose:        This Command is designed to:
                Repack UVs of current selected mesh in RizomUV and get the updated data back in Modo.
...
Cmd_Name = "smo.LL.RIZOMUV.Task.Repack"
...
print("RizomUV Livelink Python Module is located in:\n", ruvl_module_path(get_ruvpath()))
#################
...
def launchlink(export_path):
    ...
    def riz_basic_repack(mapres, iteration, OptiDistort, Opti_iter, Dist_Range):
        """
        Repack the current Model using a target preset texture map resolution / level like the one for LOD.
        You can also Repack to a new UV Set if needed like for Lightmaps

        :param mapres:          Repack the current Model using a target preset texture map resolution / level like the one for LOD.
            Map size in pixel
            0 = 4096
            1 = 2048
            2 = 1024
            ...
            4 = 256
            5 = 128
        :param iteration:          Iteration count for the Repack
        :param OptiDistort:         FixDistortion by applying a UV Optimize
        :param Opti_iter:          FixDistortion Optimize iteration count
        :param Dist_Range:         FixDistortion range span around ideal 1.0 value to select Distorted UV Islands
        :return:
        """

        pack_iter = int(iteration)
        if 0 <= iteration <= 2:
            pack_iter = 2
        if 2 < iteration <= 4:
            pack_iter = 4
        ...
        if 32 < iteration <= 64:
            pack_iter = 64
        print("Packing iteration set to %s" % pack_iter)
        link.IslandGroups({'Mode': "SetGroupsProperties", 'WorkingSet': "Visible", 'GroupPaths': ["RootGroup"], 'Properties': {'Pack': {'MaxMutations': pack_iter}}})
        ...
        # # it print a lot of data on each link.Get
        # SelIsland = []
        # for item in IslandID_s_List:
        ...
        ####################
        # RePack Islands for correcting all Undistorted Islands
        # Set the Packing IterationCount
        link.IslandGroups({'Mode': "SetGroupsProperties", 'WorkingSet': "Visible", 'GroupPaths': ["RootGroup"],
                           'Properties': {'Pack': {'MaxMutations': pack_iter}}})
        link.Select(
            {'PrimType': "Island", 'WorkingSet': "Visible", 'IslandGroupMode': "Group", 'Select': True, 'All': True})
        link.Pack({'RootGroup': "RootGroup", 'WorkingSet': "Visible&Selected", 'ProcessTileSelection': False,
                   'RecursionDepth': 1, 'Translate': True, 'AuxGroup': "RootGroup", 'LayoutScalingMode': 0})
    ...
    try:
        ...
        riz_basic_repack(RUVL_UserVal_PackMapRes, RUVL_UserVal_PackIteration, RUVL_UserVal_PackOpti, RUVL_UserVal_PackOptiIter, RUVL_UserVal_PackOptiDistRange)
        # # Pack full mesh with specific parameters
        # link.IslandGroups({'Mode': "SetGroupsProperties", 'WorkingSet': "Visible", 'GroupPaths': ["RootGroup"], 'Properties': {'Pack': {'MaxMutations': RUVL_UserVal_PackIteration}}})
        # link.IslandGroups({'Mode': "SetGroupsProperties", 'WorkingSet': "Visible", 'GroupPath': "RootGroup", 'Properties': {'Pack': {'MapResolution': RUVL_UserVal_PackMapRes}}})

        # link.IslandGroups({'Mode': "DistributeTilesContent", 'WorkingSet': "Visible&UnLocked", 'FreezeIslands': True, 'UseTileLocks': True, 'UseIslandLocks': True, 'GroupPaths': ["RootGroup"]})
        # link.Pack({'RootGroup': "RootGroup", 'WorkingSet': "Visible", 'ProcessTileSelection': False, 'RecursionDepth': 1, 'Translate': True, 'AuxGroup': "RootGroup", 'LayoutScalingMode': 0})
...
class SMO_LL_RIZOMUV_Task_Repack_Cmd(lxu.command.BasicCommand):
    def __init__(self):
    ...
    def cmd_UserName(self):
        return 'SMO LL RIZOMUV 2022.2 - Task Repack'

    def cmd_Desc(self):
        return 'Repack UVs of current selected mesh in RizomUV and get the updated data back in Modo.'
    ...
    def findRizomUVPath(self):
        ...
        # default_path = 'C:\Program Files\Rizom Lab\RizomUV 2021\\rizomuv.exe'
        default_path = 'C:\Program Files\Rizom Lab\RizomUV 2022\\rizomuv.exe'
...
lx.bless(SMO_LL_RIZOMUV_Task_Repack_Cmd, Cmd_Name)
