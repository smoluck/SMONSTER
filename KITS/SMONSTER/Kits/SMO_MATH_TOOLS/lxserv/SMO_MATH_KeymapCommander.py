...
SMO_MATH_TOOLS_HOTKEYS = [
    {
        "contexts": [
            ["schematic", "(stateless)", ".anywhere", "(contextless)", "attr.formPopover {SMO_MATH_NODES_PM:sheet}"]
        ],
        "key": "numericplus",
        "command": "attr.formPopover {SMO_MATH_NODES_PM:sheet}",
        "name": "MATH TOOLS Pie Menu",
        "info": "Numpad +"
    }

]

# max = len(SMO_MATH_TOOLS_HOTKEYS)
# lx.out(max)


class SMO_MATH_KeymapCmdClass(SmoCommanderClass):
    def commander_arguments(self):
        args = []
        for n, hotkey in enumerate(SMO_MATH_TOOLS_HOTKEYS):
            args.append({
                'name':str(n),
    ...
    def commander_execute(self, msg, flags):
        ...
        # modo.dialogs.alert("Mapped SMO MATH TOOLS Hotkeys", "Mapped %s SMO MATH TOOLS hotkeys. See Help > Smonster Hotkey Reference" % n)

lx.bless(SMO_MATH_KeymapCmdClass, "smo.MATH.MapDefaultHotkeys")


class RemoveSMO_MATH_KeymapCmdClass(SmoCommanderClass):

    def commander_execute(self, msg, flags):
        for hotkey in SMO_MATH_TOOLS_HOTKEYS:
            key = hotkey["key"]
        ...
        modo.dialogs.alert("Reverted SMO MATH TOOLS Hotkeys", "Reverted %s SMO MATH TOOLS hotkeys to defaults." % len(SMO_MATH_TOOLS_HOTKEYS))

lx.bless(RemoveSMO_MATH_KeymapCmdClass, "smo.MATH.UnmapDefaultHotkeys")


class ClearSMO_MATH_KeymapCmdClass(SmoCommanderClass):
    ...
    def commander_execute(self, msg, flags):
        ...
        modo.dialogs.alert("Cleared SMO MATH TOOLS Hotkeys", "Cleared %s SMO MATH TOOLS hotkeys attribution." % len(SMO_MATH_TOOLS_HOTKEYS))

lx.bless(ClearSMO_MATH_KeymapCmdClass, "smo.MATH.ClearHotkeys")