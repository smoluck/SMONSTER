...
SMO_MIFABOMA_HOTKEYS = [
    {
        "contexts": [
            [".global", "(stateless)", ".anywhere", "(contextless)", "attr.formPopover {SMO_MI-FA-BO-MA_PM_SH:sheet}"]
        ],
        "key": "ctrl-alt-m",
        "command": "attr.formPopover {SMO_MI-FA-BO-MA_PM_SH:sheet}",
        "name": "MIFABOMA: Pie Menu",
        "info": "Ctrl Alt M"
    }

]

# max = len(SMO_MIFABOMA_HOTKEYS)
# lx.out(max)


class SMO_MIFABOMA_KeymapCmdClass(SmoCommanderClass):
    def commander_arguments(self):
        args = []
        for n, hotkey in enumerate(SMO_MIFABOMA_HOTKEYS):
            args.append({
                'name':str(n),
...
lx.bless(SMO_MIFABOMA_KeymapCmdClass, "smo.MIFABOMA.MapDefaultHotkeys")


class RemoveSMO_MIFABOMA_KeymapCmdClass(SmoCommanderClass):

    def commander_execute(self, msg, flags):
        for hotkey in SMO_MIFABOMA_HOTKEYS:
            key = hotkey["key"]
        ...
        modo.dialogs.alert("Reverted SMO MIFABOMA Hotkeys", "Reverted %s SMO MIFABOMA hotkeys to defaults." % len(SMO_MIFABOMA_HOTKEYS))

lx.bless(RemoveSMO_MIFABOMA_KeymapCmdClass, "smo.MIFABOMA.UnmapDefaultHotkeys")


class ClearSMO_MIFABOMA_KeymapCmdClass(SmoCommanderClass):
    ...
    def commander_execute(self, msg, flags):
        ...
        modo.dialogs.alert("Cleared SMO MIFABOMA Hotkeys", "Cleared %s SMO MIFABOMA hotkeys attribution." % len(SMO_MIFABOMA_HOTKEYS))

lx.bless(ClearSMO_MIFABOMA_KeymapCmdClass, "smo.MIFABOMA.ClearHotkeys")