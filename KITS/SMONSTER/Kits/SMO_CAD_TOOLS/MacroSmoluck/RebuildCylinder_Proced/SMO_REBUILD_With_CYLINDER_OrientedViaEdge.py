...
Purpose:        This script is designed to:
                Rebuild the selected Volume with just a Cylinder that 
                got the same size as the Bounding Box of this Volume.

                Select at least 2 edge1 and run the script
                (it will select the EdgeRing , Convert the selection to Poly,
                and replace all the connected Polygons to this Polyloop)
...
if TotalSafetyCheck == TotalSafetyCheckTrueValue:
    ...
    lx.eval(
        'item.channel CapMode %s' % CYLINDER_CAPMODE)  # Cap Mode:     0 = NoCap ### 1 = NGONCap ### 2 = QuadsCap ### 3 = RadialCap
