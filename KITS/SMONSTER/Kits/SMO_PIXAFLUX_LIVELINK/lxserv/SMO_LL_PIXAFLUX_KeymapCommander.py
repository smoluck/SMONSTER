...
SMO_PIXAFLUX_LIVELINK_HOTKEYS = [
    {
        "contexts": [
            [".global", "(stateless)", ".anywhere", "(contextless)", "attr.formPopover {SMO_LL_PIXAFLUX_PM_SH:sheet}"]
        ],
        "key": "ctrl-alt-shift-p",
        "command": "attr.formPopover {SMO_LL_PIXAFLUX_PM_SH:sheet}",
        "name": "PIXAFLUX LiveLink: Popover Menu",
        "info": "Ctrl Alt Shift P"
    }

]

# max = len(SMO_PIXAFLUX_LIVELINK_HOTKEYS)
# lx.out(max)


class SMO_PIXAFLUX_LIVELINK_KeymapCmdClass(SmoCommanderClass):
    def commander_arguments(self):
        args = []
        for n, hotkey in enumerate(SMO_PIXAFLUX_LIVELINK_HOTKEYS):
            args.append({
                'name':str(n),
    ...
    def commander_execute(self, msg, flags):
        ...
        # modo.dialogs.alert("Mapped SMO PIXAFLUX LiveLink Hotkeys", "Mapped %s SMO PIXAFLUX LiveLink hotkeys. See Help > Smonster Hotkey Reference" % n)

lx.bless(SMO_PIXAFLUX_LIVELINK_KeymapCmdClass, "smo.LL.PIXAFLUX.MapDefaultHotkeys")


class RemoveSMO_PIXAFLUX_LIVELINK_KeymapCmdClass(SmoCommanderClass):

    def commander_execute(self, msg, flags):
        for hotkey in SMO_PIXAFLUX_LIVELINK_HOTKEYS:
            key = hotkey["key"]
        ...
        modo.dialogs.alert("Reverted SMO PIXAFLUX LiveLink Hotkeys", "Reverted %s SMO PIXAFLUX LiveLink hotkeys to defaults." % len(SMO_PIXAFLUX_LIVELINK_HOTKEYS))

lx.bless(RemoveSMO_PIXAFLUX_LIVELINK_KeymapCmdClass, "smo.LL.PIXAFLUX.UnmapDefaultHotkeys")


class ClearSMO_PIXAFLUX_LIVELINK_KeymapCmdClass(SmoCommanderClass):
    ...
    def commander_execute(self, msg, flags):
        ...
        modo.dialogs.alert("Cleared SMO PIXAFLUX LiveLink Hotkeys", "Cleared %s SMO PIXAFLUX LiveLink hotkeys attribution." % len(SMO_PIXAFLUX_LIVELINK_HOTKEYS))

lx.bless(ClearSMO_PIXAFLUX_LIVELINK_KeymapCmdClass, "smo.LL.PIXAFLUX.ClearHotkeys")