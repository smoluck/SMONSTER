...
class SMO_MIFABOMA_RadialSweep_CountByUser_Cmd(lxu.command.BasicCommand):
    ...
    def basic_Execute(self, msg, flags):
        ...
        lx.eval("user.defNew name:RadialSweepSideCount type:integer life:momentary")
        #Set the title name for the dialog window
        lx.eval('user.def RadialSweepSideCount dialogname "SMO RadialSweep CountByUser"')
        #Set the input field name for the value that the users will see
        lx.eval("user.def RadialSweepSideCount username {Enter side count here}")
        #The '?' before the user.value calls a popup to have the user set the value
        lx.eval("?user.value RadialSweepSideCount")
