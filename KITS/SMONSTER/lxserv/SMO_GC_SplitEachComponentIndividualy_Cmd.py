# python
"""
# Name:         SMO_GC_SplitEachComponentIndividualy_Cmd.py
# Version:      1.0
#
...
class SMO_GC_SplitEachComponentIndividualy_Cmd(lxu.command.BasicCommand):
    ...
    def basic_Execute(self, msg, flags):
        ...
        Incremental_Save_Mode = self.dyna_Int (1)   # Incremental saving at each iteration. (sort of Safe Mode)
        ...
        if Component_Mode == 0 :
            ...
            for steps in range(MaxSteps) :
                ...
                if TotalVertex > 0 :
                    ...
                    lx.eval('select.element layer:%s type:vertex mode:add index:1' % TargetMesh)
                    lx.eval('cut')
        ...
        if Component_Mode == 2 :
            ...
            for steps in range(MaxSteps) :
                ...
                #lx.out('Count Selected Edge',TotalPolyInMesh)
                
                if TotalPoly > 0 :
                    lx.eval('select.type polygon')
                    lx.eval('select.drop polygon')
                    # Select the first poly.
                    lx.eval('select.element layer:%s type:poly mode:add index:1' % TargetMesh)
                    lx.eval('cut')
                    lx.eval('select.subItem %s set mesh;replicator;meshInst;camera;light;txtrLocator;backdrop;groupLocator;replicator;surfGen;locator;falloff;deform;locdeform;weightContainer;morphContainer;deformGroup;deformMDD2;ABCStreamingDeformer;morphDeform;itemInfluence;genInfluence;deform.push;deform.wrap;softLag;ABCCurvesDeform.sample;ABCdeform.sample;force.root;baseVolume;chanModify;itemModify;meshoperation;chanEffect;defaultShader;defaultShader 0 0' % OutputUniqueName)
                    lx.eval('paste')
                    lx.eval('select.type item')
