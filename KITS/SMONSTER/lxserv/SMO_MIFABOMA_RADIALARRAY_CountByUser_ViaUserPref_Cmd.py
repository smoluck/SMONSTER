#python
#---------------------------------------
# Name:         SMO_MIFABOMA_RADIALARRAY_CountByUser_ViaUserPref_Cmd.py
# Version:      1.0
#
...
class SMO_MIFABOMA_RadialArray_CountByUser_ViaUserPref_Cmd(lxu.command.BasicCommand):
    ...
    def basic_Execute(self, msg, flags):
        ...
        if PolyMode == True or EdgeMode == True:
        ...
        lx.eval("user.defNew name:RadialArrayCount type:integer life:momentary")
        #Set the title name for the dialog window
        lx.eval('user.def RadialArrayCount dialogname "SMO RadialArray Count By User"')
        #Set the input field name for the value that the users will see
        lx.eval("user.def RadialArrayCount username {Enter count here}")
        #The '?' before the user.value calls a popup to have the user set the value
        lx.eval("?user.value RadialArrayCount")
        #Now that the user set the value, i can query it
        user_inputARRAY_COUNT = lx.eval("user.value RadialArrayCount ?")
        ...
        ARRAY_COUNT = user_inputARRAY_COUNT             # Count 4
        ...
        if ItemMode == 0 and VertMode == 0 and EdgeMode == 0 and PolyMode == 1:
            for item in SelectedMeshes:
                ...
                lx.eval('tool.setAttr gen.helix sides %s' % user_inputARRAY_COUNT)
