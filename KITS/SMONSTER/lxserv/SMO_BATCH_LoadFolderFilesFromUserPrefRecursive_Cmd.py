# python
"""
Name:         SMO_BATCH_LoadFolderFilesFromUserPrefRecursive_Cmd.py
...
Cmd_Name = "smo.BATCH.LoadFolderFilesFromUserPrefRecursive"
# smo.BATCH.LoadFolderFilesFromUserPrefRecursive


def filterlistbyformat(Target_Path: str, Targets_List: list, Extension: str):
    Fileformat = Extension.lower()
    FBXPathList = []
    CurratedFBXPathList = []

    #### Limit the Depth of the loop
    # limit = 20
    # for item in Targets_List[:limit]:
    #### Limit the Depth of the loop

    for item in Targets_List:
        # if '.fbx' in fbx.lower():                     this one will accept .Fbx .FBx .fbX, etc, because it's case insensitive
        if item.endswith("." + Fileformat.lower()) or item.endswith(("." + Fileformat.upper())):  # this one will only accept .fbx or .FBX exactly
            finalPath_AbsPath = os.path.abspath(item)
    ...
    ## Print List of files Deteced in the given Target_Path
    Targets_List = []
    print('Target_Path (absolute) = ' + os.path.abspath(Target_Path))

    ### This part write too many SMO_BATCH_RecursiveSearchFilepathList.txt on each folder which is not intended
    # for root, subdirs, files in os.walk(Target_Path):
    #     print('--\nroot = ' + root)
    #     list_file_path = os.path.join(root, 'SMO_BATCH_RecursiveSearchFilepathList.txt')
    ...
    ### Needs Rewrite to create only one SMO_BATCH_RecursiveSearchFilepathList.txt in Target_Path
    # with open(os.path.join(Target_Path, 'SMO_BATCH_RecursiveSearchFilepathList.txt'), 'wb') as log:
    #     for root, subdirs, files in os.walk(Target_Path):
    #         print('--\nroot = ' + root)
    #         list_file_path = os.path.join(root, 'SMO_BATCH_RecursiveSearchFilepathList.txt')
    #         # print('list_file_path = ' + list_file_path)
    ...
    #                     Targets_List.append(file_path)
    #                     # print('\t- file %s (full path: %s)' % (filename, file_path))
    #     log.write('The file %s contains:\n'.encode('utf-8'))
    #
    # with open(os.path.join(Target_Path, 'SMO_BATCH_RecursiveSearchFilepathList.txt'), 'wb') as log:
    #     for line in Targets_List:
    #         log.write(line.encode('utf-8'))
    #         list_file.write(b'\n')

    return CurratedFBXPathList


class SMO_BATCH_LoadFolderFilesFromUserPrefRecursive_Cmd(lxu.command.BasicCommand):
    def __init__(self):
    ...
    def basic_Execute(self, msg, flags):
        ...
        for root, subdirs, files in os.walk(Target_Path):
            ...
            ## Write to file my-directory-list.txt in root all the filepath
        ...
        if InputFileFormat == "DXF":
            CurratedDXFPathList = filterlistbyformat(Target_Path, Targets_List, "DXF")
            # for item in CurratedDXFPathList:
            #     print(item)

            #################################################################################
            #### Code embeded in function filterlistbyformat()
            # DXFPathList = []
            # CurratedDXFPathList = []
            # for dxf in os.listdir(Target_Path):
            #     # if '.fbx' in fbx.lower():                     this one will accept .Fbx .FBx .fbX, etc, because it's case insensitive
            #     if ".dxf" in dxf or ".DXF" in dxf:  # this one will only accept .dxf or .DXF exactly
            ...
            del CurratedDXFPathList

        ###########################################################################################
        # Open SVG Files
        ###########################################################################################
        if InputFileFormat == "SVG":
            CurratedSVGPathList = filterlistbyformat(Target_Path, Targets_List, "SVG")
            ...
            del CurratedSVGPathList

        ###########################################################################################
        # Open OBJ Files
        ###########################################################################################
        if InputFileFormat == "OBJ":
            CurratedOBJPathList = filterlistbyformat(Target_Path, Targets_List, "OBJ")
            ...
            del CurratedOBJPathList

        ###########################################################################################
        # Open FBX Files
        ###########################################################################################
        if InputFileFormat == "FBX":
            CurratedFBXPathList = filterlistbyformat(Target_Path, Targets_List, "FBX")
            ...
            # for item in Targets_List:
            #     # if '.fbx' in fbx.lower():                     this one will accept .Fbx .FBx .fbX, etc, because it's case insensitive
            #     if item.endswith("fbx") or item.endswith("FBX"):  # this one will only accept .fbx or .FBX exactly
            #         finalPath_AbsPath = os.path.abspath(item)
            #         # print(finalPath_AbsPath)
            #         FBXPathList.append(finalPath_AbsPath)
            #
        ...
        if InputFileFormat == "LXO":
            ...
            CurratedLXOPathList = filterlistbyformat(Target_Path, Targets_List, "LXO")
            ...
            if len(CurratedLXOPathList) > 0:
                for item in CurratedLXOPathList:
                    ...
                    if OutputFileFormat == "LXO":
                        lx.eval('scene.saveAs {%s} $LXOB false' % os.path.abspath(item))
                    if OutputFileFormat != "LXO":
                        lx.eval('smo.GC.ConvertSceneTo 1 %s' % OutputFileFormat)
                    lx.eval('!scene.close')
                    lx.out('Scene Closed')

            del (CurratedLXOPathList)

        ###########################################################################################
        # Open SLDPRT Files
        ###########################################################################################
        if InputFileFormat == "SLDPRT":
            CurratedSLDPRTPathList = filterlistbyformat(Target_Path, Targets_List, "SLDPRT")
            ...
            del (CurratedSLDPRTPathList)

    def cmd_Query(self, index, vaQuery):
        lx.notimpl()


lx.bless(SMO_BATCH_LoadFolderFilesFromUserPrefRecursive_Cmd, Cmd_Name)
