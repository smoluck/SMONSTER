...
Cmd_Name = "smo.UV.NormalizePackByUDIM"
# smo.UV.NormalizePackByUDIM %s %s 0 0

class SMO_UV_NormalizePackByUDIM_Cmd(lxu.command.BasicCommand):
    def __init__(self):
    ...
    def basic_Execute(self, msg, flags):
        ...
        lx.eval("user.defNew name:SMO_SafetyCheckNPbUDIM_UVMapCount type:boolean life:momentary")
        ## Selected UVmap Name
        ...
        NPbUDIM_UVMapName = lx.eval('user.value SMO_UV_SelectedMeshUVmapName ?')
        lx.out('Selected Mesh UV Maps Name:',NPbUDIM_UVMapName)
        
        lx.eval('smo.GC.ClearSelectionVmap 1 1')
        lx.eval('select.vertexMap %s txuv replace' % NPbUDIM_UVMapName)
        ...
        if SelectedMeshUVMapsCount > 1 :
            ...
            lx.eval('dialog.open')
            SMO_SafetyCheckNPbUDIM_UVMapCount = False
            sys.exit()
        ...
        if SelectedMeshUVMapsCount == 1 :
            SMO_SafetyCheckNPbUDIM_UVMapCount = True
        ...
        ## <----( Main Macro : )----> ##
        ################################
        if SMO_SafetyCheckNPbUDIM_UVMapCount == True :
...
lx.bless(SMO_UV_NormalizePackByUDIM_Cmd, Cmd_Name)
