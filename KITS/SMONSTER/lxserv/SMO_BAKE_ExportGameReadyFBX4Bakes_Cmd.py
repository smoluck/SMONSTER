...
Command_Name = "smo.BAKE.ExportGameReadyFBXBakes"
# smo.BAKE.ExportGameReadyFBXBakes

class SMO_BAKE_ExportGameReadyFBXBakes_Cmd (lxu.command.BasicCommand):
    def __init__ (self):
    ...
    def basic_Execute (self, msg, flags):
        ...
        ###########  Check User Values  ###########
        NameTag_LP = lx.eval1 ('user.value SMO_UseVal_BAKE_NameTagString_low ?')
        # lx.out ('Nametag for Lowpoly:    ', NameTag_LP)
        NameTag_Cage = lx.eval1 ('user.value SMO_UseVal_BAKE_NameTagString_cage ?')
        # lx.out ('Nametag for Cage:    ', NameTag_Cage)
        NameTag_HP = lx.eval1 ('user.value SMO_UseVal_BAKE_NameTagString_high ?')
        # lx.out ('Nametag for HighPoly:    ', NameTag_HP)
        
        
        AutoSave = lx.eval1 ('user.value SMO_UseVal_BAKE_AutoSaveSceneBeforeProcess ?')
        # lx.out ('AutoSave Scene before Export:    ', AutoSave)
        
        
        # Automatically Create and Set Mikk Tanget Space maps
        Bool_NRMMikkTspace = lx.eval1('user.value SMO_UseVal_BAKE_SetMikkTSpace ?')
        
        # the Freeze Subdivided Poly on High in Preferences. Incremental Save in this Mode
        Smo_Bake_FreezeHPSubdiv = lx.eval1 ('user.value SMO_UseVal_BAKE_FreezeHPSubdiv ?')
        
        # If FBX export are stored Locally in Scene Path Subfolder
        Smo_Bake_ScenePathSubfolder = lx.eval1 ('user.value SMO_UseVal_BAKE_ScenePathSubfolder ?')
        ...
        if Smo_Bake_ScenePathSubfolder == 1 :
        ...
        if Bool_NRMMikkTspace == True :
            ...
            lx.eval('smo.GC.SelectMTypMesh 0')
            lx.eval('smo.BAKE.CreateMikkTangentBasis')
            lx.eval('smo.GC.CreateDeleteSelSetFromMTypTag 1')
        ...
        if Smo_Bake_ScenePathSubfolder == 0 :
        ...
        if Smo_Bake_ScenePathSubfolder == 1 :
            # builds the complete path out of the temp dir/or/ Scene subfolder and the bakes file name
            fbx_export_path_LP = os.path.join(OutputFolder, KIT, fbx_file_name_LP)
            lx.out ('Lowpoly File Path:    ', fbx_export_path_LP)
            fbx_export_path_Cage = os.path.join(OutputFolder, KIT, fbx_file_name_Cage)
            lx.out ('Cage File Path:    ', fbx_export_path_Cage)
            fbx_export_path_HP = os.path.join(OutputFolder, KIT, fbx_file_name_HP)
            lx.out ('Highpoly File Path:    ', fbx_export_path_HP)
...
lx.bless (SMO_BAKE_ExportGameReadyFBXBakes_Cmd, Command_Name)
