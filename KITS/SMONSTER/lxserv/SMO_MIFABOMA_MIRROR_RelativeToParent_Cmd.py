...
class SMO_MIFABOMA_Mirror_RelativeToParent_Cmd(lxu.command.BasicCommand):
    ...
    def basic_Execute(self, msg, flags):
        ...
        # ------------------------------ #
        # <----( DEFINE ARGUMENTS )----> #                    # smo.MIFABOMA.MirrorRelativeToParent 1 0 1 0
        # ------------------------------ #
        args = lx.args()
        lx.out(args)
        MIRROR_AXES = self.dyna_Int (0)                     # Axes selection:       X = 0 ### Y = 1 ### Z = 2
        MERGE_VERTEX = self.dyna_Bool (1)                   # Off = 0 ### On = 1
        CLONE_TYPE = self.dyna_Int (2)                      # Clone = 0  ### Instance = 1
        CLONE_HIERARCHY = self.dyna_Int (3)                 # Clone Hierarchy Off = 0 ### Clone Hierarchy On = 1
