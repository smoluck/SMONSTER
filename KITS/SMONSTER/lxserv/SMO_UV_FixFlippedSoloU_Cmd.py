#python
#---------------------------------------
# Name:         SMO_UV_FixFlippedSoloU_Cmd.py
# Version:      1.0
# 
...
Cmd_Name = "smo.UV.FixFlippedSoloU"
# smo.UV.FixFlippedSoloU

class SMO_UV_FixFlippedSoloU_Cmd(lxu.command.BasicCommand):
    def __init__(self):
    ...
    def cmd_UserName (self):
        return 'SMO UV - Fix Flipped on U'
    ...
    def basic_Execute(self, msg, flags):
        ...
        lx.eval("user.defNew name:SMO_SafetyCheck_UVFixflippedSoloU_UVMapCount type:boolean life:momentary")
        ## Selected UVmap Name
        lx.eval("user.defNew name:UVFixflippedSoloU_UVMapName type:string life:momentary")
        #####
        ...
        UVFixflippedSoloU_UVMapName = lx.eval('user.value SMO_UV_SelectedMeshUVmapName ?')
        lx.out('Selected Mesh UV Maps Name:',UVFixflippedSoloU_UVMapName)
        
        lx.eval('smo.GC.ClearSelectionVmap 1 1')
        lx.eval('select.vertexMap %s txuv replace' % UVFixflippedSoloU_UVMapName)
        
        if SelectedMeshUVMapsCount > 1 :
            lx.eval('dialog.setup info')
            lx.eval('dialog.title {SMONSTER - UV Fix flipped on U:}')
            lx.eval('dialog.msg {Please select Only One Vertex Map and run that script again.}')
            lx.eval('dialog.open')
            SMO_SafetyCheck_UVFixflippedSoloU_UVMapCount = False
            sys.exit()
        ...
        if SelectedMeshUVMapsCount == 1 :
            SMO_SafetyCheck_UVFixflippedSoloU_UVMapCount = True
        ...
        lx.eval('select.uvOverlap {%s} false false true false false false' % UVFixflippedSoloU_UVMapName)
        ...
        ## <----( Main Macro )----> ##
        ##############################
        if SMO_SafetyCheck_UVFixflippedSoloU_UVMapCount == True :
...
lx.bless(SMO_UV_FixFlippedSoloU_Cmd, Cmd_Name)
