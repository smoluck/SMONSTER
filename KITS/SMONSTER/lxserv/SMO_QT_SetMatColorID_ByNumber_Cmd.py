# python
"""
Name:         SMO_GC_SetMatColorID_ByNumber_Cmd.py
...
# by checking the presence of the Channel "MatColorIDGlobalCount"
...
def IsThereTargetGrpMaskColorIDMatTag(ListGrpMaskColorID, IDNum):
    scn = modo.scene.current()
    GrpMaskAlreadyThere = False
    ListTargetGrpMaskState = []
    # print(getTargetGrpMaskName(IDNum))
    IsThereTargetGrpMaskColorIDMatTag = False
    for item in ListGrpMaskColorID:
        scn.select(item)
        for mask in scn.selectedByType('mask'):
            # print(mask.name)
            if mask.name == (getTargetGrpMaskName(IDNum) + " " + "(Material)"):
                IsThereTargetGrpMaskColorIDMatTag = True
                ListTargetGrpMaskState.append(IsThereTargetGrpMaskColorIDMatTag)
...
# print(IsThereTargetGrpMaskColorIDMatTag(ListGrpMaskColorIDSelSet(IDNum), IDNum))


# test if there is already MaxColorID in the "Base Shader" Render Item by
# checking the presence of the Channel "MatColorIDGlobalCount"
...
class SMO_GC_SetMatColorID_ByNumber_Cmd(lxu.command.BasicCommand):
    ...
    def basic_Execute(self, msg, flags):
        ...
        print('---')
        GrpMaskExist = IsThereTargetGrpMaskColorIDMatTag(ListGrpMaskColorIDSelSet(IDNum), IDNum)[0]
        # print(GrpMaskExist)
