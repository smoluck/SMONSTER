...
class IslandCount_Cmd(lxu.command.BasicCommand):
    ...
    def get_IslandCount(self):
        ...
        layer_scan = lx.object.LayerScan(
            layer_svc.ScanAllocate(lx.symbol.f_LAYERSCAN_ACTIVE | lx.symbol.f_LAYERSCAN_MARKPOLYS))
        if layer_scan.test():
    ...
    def basic_Execute(self, msg, flags):
        ...
        msg = 'Found %s polygon island%s across %s mesh%s.' % (
        islands, '' if islands == 1 else 's', layer_count, '' if layer_count == 1 else 'es')
