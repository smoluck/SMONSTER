...
CMD_NAME = 'smo.MATH.RemappingFCL'


FORMS = [
    {
        "label": "MATH TOOLS Pie Menu",
        "recommended": "numericplus",
        "cmd": "attr.formPopover {SMO_MATH_NODES_PM:sheet}"
    }
...
class SMO_MATH_TOOLS_KeymapCmdListClass(lxifc.UIValueHints):
...
class SMO_MATH_KeymapCmdClass(lxu.command.BasicCommand):
    def __init__(self):
    ...
    def arg_UIValueHints(self, index):
        if index == 0:
            return SMO_MATH_TOOLS_KeymapCmdListClass(list_commands())
...
lx.bless(SMO_MATH_KeymapCmdClass, CMD_NAME)
