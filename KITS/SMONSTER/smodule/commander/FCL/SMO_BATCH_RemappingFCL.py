...
CMD_NAME = 'smo.BATCH.RemappingFCL'

FORMS = [
    {
        "label": "BATCH: Popover Menu",
        "recommended": "shift-f5",
        "cmd": "attr.formPopover {SMO_BATCH_PO_SH:sheet}"
    }
...
class SMO_BATCH_KeymapCmdListClass(lxifc.UIValueHints):
...
class SMO_BATCH_KeymapCmdClass(lxu.command.BasicCommand):
    def __init__(self):
    ...
    def arg_UIValueHints(self, index):
        if index == 0:
            return SMO_BATCH_KeymapCmdListClass(list_commands())
...
lx.bless(SMO_BATCH_KeymapCmdClass, CMD_NAME)
